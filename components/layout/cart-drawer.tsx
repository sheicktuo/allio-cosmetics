...
export default function CartDrawer() {
  ...
  // Close when navigating to cart or checkout
  useEffect(() => {
    if (pathname === "/cart" || pathname === "/checkout") setOpen(false)
  }, [pathname])
            ...
            <div className="divide-y divide-border">
              {items.map((item) => (
                <div key={item.id} className="flex items-start gap-3 px-5 py-4">
                  {/* Emoji thumbnail */}
                  <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-primary/15 via-muted to-accent flex items-center justify-center text-2xl flex-shrink-0">
                    {item.emoji}
                  </div>
